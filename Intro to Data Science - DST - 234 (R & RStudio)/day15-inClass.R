...
mtcars |> ggplot(aes(x=wt,y=mpg,color=as.factor(cyl))) + geom_point() + geom_smooth(method="lm")

mpg |> ggplot(aes(x=displ,y=hwy,color=as.factor(class))) + geom_point() + geom_smooth(method="lm",se=FALSE)
?geom_smooth
