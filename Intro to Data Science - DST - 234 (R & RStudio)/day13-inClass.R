...
# Makes a highlighted version of matches (helpful for just viewing results)
x |> str_view("an")

# In practice you probably will use str_subset returns pieces that match:
x |> str_subset("an")

# The period (.) returns pieces of strings around your match which matches
# any character (except a newline):
x |> str_subset(".a.")  # Find the a and match 1 character left and right
x |> str_subset("..a..")  # Two characters L and R
x |> str_subset("..a")  #  Two characters L

# String functions: matching start / end
x |> str_subset("^a")   # match the start
x |> str_subset("a$")   # match the end

# Match Exactly:
x2 <- c("apple pie", "apple", "apple cake")
x2 |> str_subset("apple")
x2 |> str_subset("^apple$")   # Exact matches



# The next step up in complexity is figuring out how to match special characters
# (in particular a period (.) and slash (\) ):
...
# 0: Glimpse the words data frame
glimpse(words)

# 1. Solution:
words |>
  str_subset() ### INSERT CODE INSIDE str_subset()

# 2. Solution
words |>
  str_subset() ### INSERT CODE INSIDE str_view()
