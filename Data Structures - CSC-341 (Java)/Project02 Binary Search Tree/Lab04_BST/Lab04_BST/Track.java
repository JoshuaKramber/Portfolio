...
public class Track implements Comparable<Track> {
  ...
  */
  ...
  public boolean equals(Object other) {
    //checks if the values are equal then returns or continues
    if (this==other){
       return true;
    }
    //checks if the other object is of the class track
    if (!(other instanceof Track)){
      return false;
    }
    //casts the other object as a track
    Track newItem = (Track)other;
    //compares the strings of the titles for each track
    if (this.title().compareTo(newItem.title())==0){
      return true;
    }
    return false;
  }
