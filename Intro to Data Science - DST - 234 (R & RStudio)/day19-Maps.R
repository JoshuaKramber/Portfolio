...
ggplot() +
  geom_polygon(data = usa, aes(x=long, y = lat,group=group), fill = "purple", color = "green",alpha = 0.3) +
  coord_fixed(1.3)
...
ggplot() + geom_polygon(data = states, aes(x=long, y = lat,group=group),fill="blue",color="red") +
  coord_fixed(ratio = 1.3)
...
state_x77_df <- state.x77 |>
  data.frame() |>   # Make this into a data frame
...
ggplot() + geom_polygon(data = mn, aes(x=long, y = lat, group = group)) +
  coord_fixed(1.3)
...
ggplot() + geom_polygon(data = mnwi, aes(x=long, y = lat, fill = "red" ,group = group),color="black") +
  coord_fixed(1.3) + 
  guides(fill="none")
...
### --> FILL IN THE CODE WITH YOUR SOLUTION
ggplot(data = world) + geom_polygon(aes(x = long, y = lat, fill = region, group = group), color = "black") +
  coord_fixed(1.3) +
  guides(fill="none")
...
### --> FILL IN THE CODE WITH YOUR SOLUTION
arctic <- map_data("world2", region="UK")
ggplot(data=arctic)+
geom_polygon(aes(x = long, y = lat, fill = subregion, group = group), color = "black") +
  coord_fixed(7) +
  guides(fill="none")
